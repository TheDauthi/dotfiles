...
language_env="pyenv"
virtualenv_root="$HOME/.pyenv"
language_env_repo="https://github.com/pyenv/pyenv.git"
plugins=(
...
default_packages=(pip)
default_package_file_name="default-packages"

PATH="$virtualenv_root/bin:$PATH"

get_latest_stable_version() {
    latest_stable_version=$($language_env install -l | sed 's/^\s\+//' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+$' | sort -V | tail -n 1)
}
